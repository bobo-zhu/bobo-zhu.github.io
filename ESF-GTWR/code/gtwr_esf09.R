...
data <- data %>%
  mutate(across(c("Zat","Zlw","Zghf","Zws","Zet","Zsw","Ztp","Zssr"), 
                ~ as.numeric(as.character(.))))

data <- data %>%
  mutate(across(c("Zat","Zlw","Zghf","Zws","Zet","Zsw","Ztp","Zssr"), scale))
...
image_path <- paste('D:/zhudaoyong/esf_scripts/esf-gtwr/esf-gtwr09.Rdata')
save.image(image_path)

names(index_result) <- c('r.adjusted', 'aic', 'aicc', 'rmse', 'bw')
write.csv(index_result, file=paste('D:/zhudaoyong/esf_scripts/esf-gtwr/results09.csv'))
